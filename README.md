...
+ コンバート時に、「町域」が、block と street に分離されていることに注意。
    + 具体的には「町域」に '（' と '）' で囲んで追記されている部分を street として分離し、それ以外を block として残している。
+ street および street_kana の表示には注意が必要である。
    ...
    + 要するに、street および street_kana は、そのまま使用すべき「住所」の構成要素ではなくて、「住所」と郵便番号の対応関係を説明する「但し書き」として表示すべきものである。
